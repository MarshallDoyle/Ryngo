...
| **Phase 10.warning-unlock — 6 AST-based warnings** | claude | shipped | `mvp/lib/warnings-ast.js` (new) with await-in-loop, unreachable-code, mutation-of-param, switch-without-default, function-defined-in-loop, unhandled-promise-rejection. Runs via `attachAstWarnings` post-pass in `parsers/index.js#parseFile`. Total shipped warnings: 33. |
...
| Phase 5.1.0 + 5.1.1 — tree-sitter runtime + TS/JS extractor | claude | shipped, dormant | `mvp/lib/parsers/tree-sitter-runtime.js` + `mvp/lib/parsers/queries/ts/*.scm` (4 files) + `mvp/lib/parsers/ts-tree-sitter.js` + dispatch in `mvp/lib/parsers/index.js` + Dockerfile build tools. Feature-flag: `RYNGO_PARSERS=tree-sitter` opts in; default = regex, zero corpus drift. |
