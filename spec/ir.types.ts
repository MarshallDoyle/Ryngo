/**
 * codegraph IR — TypeScript types (schema v0.1.0)
 *
 * Companion files:
 *   - ir-schema.md       (design rationale + worked example)
 *   - ir.schema.json     (JSON Schema, draft 2020-12; the normative validator)
 *
 * These types intentionally mirror the JSON Schema. Where the JSON Schema
 * uses `oneOf` over a discriminator field, the TypeScript equivalent is a
 * discriminated union. Forward-compatibility escape hatches (`x-*` enum
 * extensions, "Unknown" variants) are encoded via `(string & {})` openings
 * so that current values get autocomplete while still typechecking unknown
 * future values.
 *
 * Convention: every enum-like string field has a "known" union joined with
 * `(string & {})` so future analyzer extensions are accepted without a type
 * error but known values still autocomplete.
 */
...
export type SchemaVersion = `${number}.${number}.${number}` | `${number}.${number}.${number}-${string}`;
...
/** Tier discriminator. Open via `x-*` for forward compatibility. */
export type Tier =
  | "service"
  | "module"
  | "type"
...
/** Common fields. Concrete tiers narrow these. */
interface NodeBase {
  id: NodeId;
...
export function isKnownTier(t: Tier): t is Exclude<Tier, `x-${string}`> {
...
] as const satisfies ReadonlyArray<Exclude<Tier, `x-${string}`>>;
